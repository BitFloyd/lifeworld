...
def train_for_n_captions(dset_train, dset_middle_empty_train, train_captions, adversary_model, gan_merged_model,
                         ...
                         losses_per_frq,
                         nb_epoch=50000, plt_frq=100, batch_size=50, predict_frq=1000, run_num=10, m_train=False,
                         cp=0.1, adversary_model_half=None):
...
def DCGAN_avec_captions_slow_converge_avec_inception(num_runs=10, n_epoch=1):
    ...
    print "GET GAN MODEL"
    gan_merged_model, adversary_model = model_zoo.DC_caption_LSTM_inception_only(shape_img,
                                                                                 [32, 64, 128, 256],
                                                                                 noise=True)
    ...
    print "PARAMETERS AM"
    print adversary_model.count_params()
    print "PARAMETERS FM"
    ...
    for i in range(0, num_runs):
        ...
        # Refeed latest predictions as input
        # dset_middle_empty_train = predictions

        # Strengthen adversary to stabilize.
        strengthen_adversary(dset_train, predictions, adversary_model, strength=0.01)
        ...
        del (predictions)
        full_model.save_weights(model_weights)
        full_model.save('/usr/local/data/sejacob/lifeworld/data/inpainting/models/caption_full_model_inception_only.h5')

    print "FINISHED ALL RUNS"


def DCGAN_captions_LSTM_train_both(num_runs=10, n_epoch=1):
    # Get data as numpy mem-map to not overload the RAM
    plt.ioff()
    print "GET_DATASETS"
    save_path = '/usr/local/data/sejacob/lifeworld/data/inpainting/pkls/'
    ...
    for i in range(0, num_runs):
        ...
        print "SAVE IMAGES"
        write_path = '/usr/local/data/sejacob/lifeworld/data/inpainting/predictions/exp08_dcgan_captions_run_train' + \
                     str(i + 1).zfill(2) + '/'
        ...
        print "SAVE IMAGES"
        write_path = '/usr/local/data/sejacob/lifeworld/data/inpainting/predictions/exp08_dcgan_captions_run_test' + \
                     str(i + 1).zfill(2) + '/'
